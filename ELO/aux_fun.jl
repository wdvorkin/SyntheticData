...
function create_OPF_records(set)
    OPF_δ = Dict(:c => CSV.File("data/OPF_records_c.csv",header=0) |> Tables.matrix,
                 ...
                 :f̅ => CSV.File("data/OPF_records_f.csv",header=0) |> Tables.matrix)
    # OPF_records
    OPF_records = Dict()
    Random.seed!(200)
    OPF_f = rand(Uniform(0.85,1.15),net[:E],set[:S])
    Random.seed!()
    for i in 1:set[:S]
        network = deepcopy(net)
        network[:c1] = OPF_δ[:c][:,i]
        network[:p̅] = net[:p̅] .* OPF_δ[:p][:,i]
        network[:f̅] = net[:f̅] .* OPF_δ[:f̅][:,i]
        OPF_records[i] = network
    end
    return OPF_records
end
function opf_feas_opt_test(OPF_records,set,d)
    inf_coutner = [0]
    sub_opt = zeros(set[:S])
    for i in 1:set[:S]
        sol_real = OPF(OPF_records[i],net[:d])

        sol_opf = OPF(OPF_records[i],d)
        sol_opf[:status] != "OPTIMAL" ? push!(inf_coutner,1) : NaN

        sol_opf = OPF_R(OPF_records[i],d)
