function OPF(net,φ)
    ...
    # OPF equations
    @constraint(model, λ, ones(net[:N])'*(p .- net[:d]) .>= 0)
    @constraint(model, μ̅, φ .>=   net[:F]*(p .- net[:d]))
    @constraint(model, μ̲, φ .>= - net[:F]*(p .- net[:d]))
...
function OPF_R(net,φ)
    ...
    # OPF equations
    @constraint(model, λ, ones(net[:N])'*(p .- net[:d]) .>= 0)
    @constraint(model, μ̅, - net[:F]*(p .- net[:d]) .>= - φ - v)
    @constraint(model, μ̲, φ + v .>= - net[:F]*(p .- net[:d]))
    @constraint(model, γ̅,  -p .>= -net[:p̅])
    @constraint(model, γ̲,  -net[:p̲] .>= -p)
...
function OPF_PP(net,φ̅_o,OPF_records,wc_cost,k_t)
    t = length(k_t)
    model = Model(optimizer_with_attributes(Gurobi.Optimizer))
    set_optimizer_attribute(model, "OutputFlag", 0)
    # upper-level variables 
    @variable(model, φ[1:net[:E]]>=0)
    @variable(model, r_φ)
    ...
    # objective function
    @objective(model, Min, 1 * r_φ +  sum(r_c[τ] for τ in 1:t))
    # objective aux constraints 
    @constraint(model, [r_φ;φ .- φ̅_o] in SecondOrderCone())
    @constraint(model, aux_con[τ=1:t], [r_c[τ];wc_cost[τ] .- OPF_records[k_t[τ]][:c1]'p[:,τ]] in SecondOrderCone())
    for τ in 1:t
        # primal constriants 
        @constraint(model, ones(net[:N])'*(p[:,τ] .- OPF_records[k_t[τ]][:d]) .== 0)
        @constraint(model, - net[:F]*(p[:,τ] .- OPF_records[k_t[τ]][:d]) .>= - φ)
        @constraint(model, φ .>= - net[:F]*(p[:,τ] .- OPF_records[k_t[τ]][:d]))
        ...
        for i in 1:net[:E]
            @constraint(model, [μ̅[i,τ] ; φ[i] .- net[:F][i,:]'*(p[:,τ] .- OPF_records[k_t[τ]][:d])] in SOS1())
            @constraint(model, [μ̲[i,τ] ; net[:F][i,:]'*(p[:,τ] .- OPF_records[k_t[τ]][:d]) .+ φ[i]] in SOS1())
        end
        for i in 1:net[:N]
